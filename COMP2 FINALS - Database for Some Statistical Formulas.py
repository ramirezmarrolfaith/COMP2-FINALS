#Caseda, Hans Martin A.
#Esclamado, Ronalyn R.
#Labide, John Paul L.
#Ramirez, Marrol Faith F.
#Sebastian, Aisle Ronna L.
#Talarion, Russell H.
...
def main():
    print('シ Welcome to Data Base of mathematical Formula シ')
    print('        ==========Main Menu=========')
    print('        |Choose|    Description    |      ♥')
    print('      ♥ |   A  |Calculate  ¾ ▲ ▒   | ♥')
    print('        |   B  |Import & Delete    |  ')
    print('  ♥     |   C  |Exit               |   ♥')
    choose = input('Choose: ')
    if choose.title() == 'A':
        calculate()
...
def calculate():
    def calculator():
        def basiccalc():
            while True:
                try:
                    ans = eval(input('Welcome to Calculator: '))
                    print(ans)
                except Exception:
                    print('Invalid Input')
                finally:
                    ans = input('Continue? press Y/y ')
                    if ans.upper() != 'Y':
                        sys.exit(calculator())
                    else:
                        basiccalc()
        ...
        def exponent():
            while True:
                try:
                    num = eval(input('Enter a Base: '))
                    num1 = eval(input('Enter a Exponent: '))
                    ans = num**num1
                    print(ans)
                except Exception:
                    print('Invalid Input')
                ...
                if ans.upper() != 'Y':
                    ...
                    exponent()
        def c_other():
            workbook = openpyxl.load_workbook('Other Calculator.xlsx')
            worksheet = workbook.active
            print('A-Z/Z-A/Find: ')
            for row in worksheet.iter_rows(values_only=True):
                print("| {:^10} | {:^20} |".format(*row))
            opt = input('A-Z/Z-A/Find/Exit: ')
            if opt.title() == 'Find':
                while True:
                    for row in worksheet.iter_rows(values_only=True):
                        print("| {:^10} | {:^20} |".format(*row))
                    name = input("Enter the name of the Formula: ")
                    found = False
                
                    for row in worksheet.iter_rows(min_row=2, min_col=1, max_col=2):  
                        name_cell, formula_cell = row
                        if name_cell.value == name:
                            formula = formula_cell.value
                            print(f"{name} is {formula}")
                            found = True
                            break
                    ...
                    if not found:
                        ...
                        continue 
                    letters_only = re.sub('[^a-zA-Z]', '', formula)
                    ...
                    if 's' in formula:
                        if letter_count == 2:
                            ...
                            ans = eval(formula)
                            print(ans)
                        opt = input('Want to try again Y/y: ')
                        if opt.upper() != 'Y':
                            break
                ...
                data_rows = list(islice(worksheet.iter_rows(values_only=True), 1, None))
                sorted_data_rows = sorted(data_rows, key=lambda row: row[0])
                print("| {:^10} | {:^20} |".format(*header_row))
                ...
                header_row = next(worksheet.iter_rows(values_only=True))
                data_rows = list(islice(worksheet.iter_rows(values_only=True), 1, None))
                sorted_data_rows = sorted(data_rows, key=lambda row: row[0],reverse=True)
                print("| {:^10} | {:^20} |".format(*header_row))
                for row in sorted_data_rows:
                    print("| {:^10} | {:^20} |".format(*row))
                opt = input('Find/Exit: ')
    ...
    def statistics():
        def centraltendency():
            def get_mean_median_mode(data):
                ...
                modes = [k for k, v in counts.items() if v == mode_counts]
                mode = modes[0] if len(modes) == 1 else None 
                return mean, median, mode
            
            data = []  
            user = input('Enter a number (seperate with comma ","): ')
            revcom = user.split(',')
            for i in revcom:
                data.append(int(i))
            mean, median, mode = get_mean_median_mode(data)
            print(data)
            print('Mean:', mean)
            print('Median:', median)
            print('Mode:', mode)
            ...
            if ans.upper() != 'Y':
                ...
                centraltendency()
        def s_other():
            workbook = openpyxl.load_workbook('Other Statistics.xlsx')
            worksheet = workbook.active
            print('A-Z/Z-A/Find: ')
            for row in worksheet.iter_rows(values_only=True):
                print("| {:^10} | {:^20} |".format(*row))
        ...
        while True:
            print('▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢▢')
            print('|Code|               Description               |')
            print('|  1 |Central Tendency (Mean, Median and Mode) |')
            print('|  2 |Other Statistics                         |')
            print('|  3 |Exit                                     |')
            choose = int(input('Chooose: '))
            if choose == 1:
                centraltendency()
            elif choose == 2:
                s_other()
            ...
            if opt.title() == 'Find':
                while True:
                    ...
                    for row in worksheet.iter_rows(min_row=2, min_col=1, max_col=2):
                        name_cell, formula_cell = row
                        if name_cell.value == name:
                            program = formula_cell.value
                            print(f"Executing {name} program")
                            found = True
                            exec(program)
                            break
...
def import_option_dev():
    def imporT():
        global lst
        def calculator_import():
            global cal_added,lst
            workbook = openpyxl.load_workbook('Other Calculator.xlsx')
            ...
            while True:
                ...
                worksheet.cell(row=new_row, column=name_col, value=name)
                worksheet.cell(row=new_row, column=formula_col, value=formula)
                opt = input('Are you sure you want to add it? Y/y: ')
                if opt.upper() == 'Y':
                    workbook.save('Other Calculator.xlsx')
                    cal_added.append(name)
                    lst = cal_added + stat_added + other_added
                opt = input('Again Y/y: ')
                if opt.upper() != 'Y':
                    imporT()
 
        def statistics_import():
            global stat_added, lst
            workbook = openpyxl.load_workbook('Other Statistics.xlsx')
            worksheet = workbook['Sheet1']
            name_col = None
            for col in worksheet.iter_cols(max_row=1):
            ...
            while True:
                ...
                opt = input('Are you sure you want to add it? Y/y: ')
                if opt.upper() == 'Y':
                    workbook.save('Other Statistics.xlsx')
                    stat_added.append(name)
                    lst = cal_added + stat_added + other_added
        ...
        def others_import():
            ...
            if name_col is None:
                name_col = worksheet.max_column + 1
                worksheet.cell(row=1, column=name_col, value='Program')


            if formula_col is None:
                formula_col = worksheet.max_column + 1
                worksheet.cell(row=1, column=formula_col, value='Code')
            cal_added = []
            stat_added = []
            other_added = []
            while True:
                name = input('Enter the name of your python file (Don\'t include the file type): ')
                filename = name + '.py'
                new_row = worksheet.max_row + 1
                ...
                worksheet.cell(row=new_row, column=name_col, value=name)
                worksheet.cell(row=new_row, column=formula_col, value=code)
        ...
        while True:
            print('Where do you wanna import your calculator/formula? ')
            print('■■■■■■■■■■■■■■■■■■■■■■■■■■■■■')
            print('|Code|       Description    |')
            print('|  1 |Calculator            |')
            print('|  2 |Statistics            |')
            print('|  3 |Other Python Program  |')
            print('|  4 |Exit                  |')
            ...
            if choose == 1:
                ...
                sys.exit(import_option_dev())
        
    def manager():
        global cal_added
        def mod_cal():
            global cal_added
            opt = input('Want to remove certain formula Y/y: ')
            if opt.upper() == 'Y':
                while True:
                    print('|    name    |')
                    for i in cal_added:
                    ...
                    for row in worksheet.iter_rows(min_row=2):
                        if row[name_col - 1].value == name_to_delete:
                            row_index = row[0].row
                            break
                    ...
                    opt = input('Are you sure you delete it? Y/y: ')
                    if opt.upper() == 'Y':
                        workbook.save('Other Calculator.xlsx')
                        temp_cal_added = []
                        for name in cal_added:
                            if name != name_to_delete:
                                temp_cal_added.append(name)
                        cal_added = temp_cal_added
            ...
            if opt.upper() == 'Y':
                while True:
                    ...
                    if opt.upper() == 'Y':
                        workbook.save('Other Statistics.xlsx')
                        temp_cal_added = []
                        for name in stat_added:
                            if name != name_to_delete:
                                temp_cal_added.append(name)
                        stat_added = temp_cal_added
            ...
            if opt.upper() == 'Y':
                while True:
                    ...
                    if opt.upper() == 'Y':
                        workbook.save('Python.xlsx')
                        temp_cal_added = []
                        for name in other_added:
                            if name != name_to_delete:
                                temp_cal_added.append(name)
                        other_added = temp_cal_added
                    opt = input('Again Y/y: ')
                    if opt.upper() != 'Y':
                        manager() 
                    else:
        ...
        while True:
            print('What module you wanna make a change?')
            print('■■■■■■■■■■■■■■■■■■■■■■■■■■■■■')
            print('|Code|       Description    |')
            print('|  1 |Calculator            |')
            print('|  2 |Statistics            |')
    ...
    while True:
        print('▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩')
        print('|Code|              Description              |')
        print('|  1 |Import in Calculator/Statistics/Others |')
        print('|  2 |Manage                                 |')
        print('|  3 |Exit                                   |')
...
main()
ndarray(_ArrayOrScalarCommon, Generic[_ShapeType, _DType_co]):
    __hash__: ClassVar[None]
    @property
